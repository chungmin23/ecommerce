...
public interface CartItemRepository extends JpaRepository<CartItem, Long>{
  ...
  @Query("select " + 
  " new org.shop.apiserver.application.dto.CartItemListDTO(ci.cino,  ci.qty,  p.pno, p.pname, p.price , pi.fileName )  " +
  " from " +
  ...
  public Long getCartFromItem( @Param("cino") Long cino);
    ...
    @Query("select new org.shop.apiserver.application.dto.CartItemListDTO(ci.cino,  ci.qty,  p.pno, p.pname, p.price , pi.fileName )  " +
  " from " + 
  "   CartItem ci inner join Cart mc on ci.cart = mc " +
  "   left join Product p on ci.product = p " +
  "   left join p.imageList pi" +
